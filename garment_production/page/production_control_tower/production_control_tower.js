frappe.pages["production-control-tower"].on_page_load = function (wrapper) {
...
frappe.pages["production-control-tower"].on_page_show = function (wrapper) {
	load_control_tower(wrapper);
};
...
function load_control_tower(wrapper) {
	...
	$parent.empty();
	$(frappe.render_template("production_control_tower")).appendTo($parent);
